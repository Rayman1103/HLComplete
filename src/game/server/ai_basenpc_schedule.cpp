	...
	case TASK_DIE:
		{
			if ( m_fSequenceFinished && pev->frame >= 255 )
			{
				...
				if ( !BBoxFlat() )
				{
					...
					// block the player on a slope or stairs, the corpse is made nonsolid. 
//					pev->solid = SOLID_NOT;
					UTIL_SetSizeHL1 ( pev, Vector ( -4, -4, 0 ), Vector ( 4, 4, 1 ) );
				}
				else // !!!HACKHACK - put monster in a thin, wide bounding box until we fix the solid type/bounding volume problem
					UTIL_SetSizeHL1 ( pev, Vector ( pev->mins.x, pev->mins.y, pev->mins.z ), Vector ( pev->maxs.x, pev->maxs.y, pev->mins.z + 1 ) );
				...
				else
				{
					...
					CSoundEntHL1::InsertSound ( bits_SOUND_CARCASS, pev->origin, 384, 30 );
				}
			}
			break;
		}
	...
	case TASK_WAIT_FOR_SCRIPT:
		{
			if ( m_pCine->m_iDelay <= 0 && gpGlobalsHL1->time >= m_pCine->m_startTime )
			{
				TaskComplete();
				...
				if ( m_fSequenceFinished )
					ClearSchedule();
				pev->framerate = 1.0;
				//ALERT( at_aiconsole, "Script %s has begun for %s\n", STRING_HL1( m_pCine->m_iszPlay ), STRING_HL1(pev->classname) );
			}
			break;
		}
	case TASK_PLAY_SCRIPT:
...
//				return;
			...
			{
				// try lateral first
				m_flMoveWaitFinished = gpGlobalsHL1->time + pTask->flData;
				TaskComplete();
			}
	...
	case TASK_WAIT_RANDOM:
		{// set a future time that tells us when the wait is over.
			m_flWaitFinished = gpGlobalsHL1->time + RANDOM_FLOAT( 0.1, pTask->flData );
			break;
		}
	case TASK_MOVE_TO_TARGET_RANGE:
		{
	...
	case TASK_GET_PATH_TO_ENEMY_CORPSE:
		{
			...
			if ( BuildRoute ( m_vecEnemyLKP - gpGlobalsHL1->v_forward * 64, bits_MF_TO_LOCATION, NULL ) )
			{
				TaskComplete();
			}
			else
	...
	case TASK_GET_PATH_TO_SPOT:
		{
			CBaseEntityHL1 *pPlayer = CBaseEntityHL1::Instance( FIND_ENTITY_BY_CLASSNAME( NULL, "player" ) );
...
case TASK_GET_PATH_TO_BESTSCENT:
	...
	case TASK_STRAFE_PATH:
		{
			Vector2DHL1	vec2DirToPoint; 
			Vector2DHL1	vec2RightSide;

			// to start strafing, we have to first figure out if the target is on the left side or right side
			UTIL_MakeVectors ( pev->angles );

			vec2DirToPoint = ( m_Route[ 0 ].vecLocation - pev->origin ).Make2D().Normalize();
			vec2RightSide = gpGlobalsHL1->v_right.Make2D().Normalize();
	...
	case TASK_WAIT_FOR_SCRIPT:
		{
			if (m_pCine->m_iszIdle)
			{
				...
				if (FStrEq( STRING_HL1(m_pCine->m_iszIdle), STRING_HL1(m_pCine->m_iszPlay)))
				{
					pev->framerate = 0;
				}
			}
...
// scheduled task. NULL if there's a problem.
//=========================================================
Task_t_hl1	*CBaseMonster :: GetTask ( void ) 
{
