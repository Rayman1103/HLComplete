...
class CSpriteHL1 : public CPointEntity
{
public:
	void Spawn( void );
	void Precache( void );
	...
	{ 
		...
		return (CBaseEntityHL1 :: ObjectCaps() & ~FCAP_ACROSS_TRANSITION) | flags; 
	}
	...
	void SpriteInit( const char *pSpriteName, const VectorHL1 &origin );

	inline void SetAttachment( edict_t_hl1 *pEntity, int attachment )
	{
		if ( pEntity )
		{
	...
	{ 
		SetThinkHL1(AnimateUntilDead); 
		pev->framerate = framerate;
		pev->dmgtime = gpGlobalsHL1->time + (m_maxFrame / framerate); 
	...
	static CSpriteHL1 *SpriteCreate( const char *pSpriteName, const VectorHL1 &origin, BOOL animate );
...
class CBeamHL1 : public CBaseEntityHL1
{
public:
	void	Spawn( void );
	void	Precache( void );
	...
	void EXPORT TriggerTouch( CBaseEntityHL1 *pOther );
	...
	inline void SetStartPos( const VectorHL1& pos ) { pev->origin = pos; }
	inline void SetEndPos( const VectorHL1& pos ) { pev->angles = pos; }
	...
	const VectorHL1 &GetStartPos( void );
	const VectorHL1 &GetEndPos( void );

	VectorHL1 Center( void ) { return (GetStartPos() + GetEndPos()) * 0.5; }; // center point of beam
...
//	void		SetObjectCollisionBox( void );
	...
	void		DoSparks( const VectorHL1 &start, const VectorHL1 &end );
	CBaseEntityHL1 *RandomTargetname( const char *szName );
	void		BeamDamage( TraceResult *ptr );
	// Init after BeamCreate()
	void		BeamInit( const char *pSpriteName, int width );
	void		PointsInit( const VectorHL1 &start, const VectorHL1 &end );
	void		PointEntInit( const VectorHL1 &start, int endIndex );
	void		EntsInit( int startIndex, int endIndex );
	void		HoseInit( const VectorHL1 &start, const VectorHL1 &direction );

	static CBeamHL1 *BeamCreate( const char *pSpriteName, int width );

	inline void LiveForTime( float time ) { SetThinkHL1(SUB_Remove); pev->nextthink = gpGlobalsHL1->time + time; }
...
class CLaserHL1 : public CBeamHL1
{
public:
	void	Spawn( void );
	void	Precache( void );
	...
	CSpriteHL1	*m_pSprite;
	int		m_iszSpriteName;
	VectorHL1  m_firePosition;
};
