...
int CFlyingMonster :: CheckLocalMove ( const VectorHL1 &vecStart, const VectorHL1 &vecEnd, CBaseEntityHL1 *pTarget, float *pflDist )
{
	...
	UTIL_TraceHull( vecStart + VectorHL1( 0, 0, 32 ), vecEnd + VectorHL1( 0, 0, 32 ), dont_ignore_monsters, large_hull, edict(), &tr );
	...
	if (pflDist)
	{
		*pflDist = ( (tr.vecEndPos - VectorHL1( 0, 0, 32 )) - vecStart ).Length();// get the distance.
	}
...
BOOL CFlyingMonster :: FTriangulate ( const VectorHL1 &vecStart , const VectorHL1 &vecEnd, float flDist, CBaseEntityHL1 *pTargetEnt, VectorHL1 *pApex )
{
...
float CFlyingMonster :: ChangeYaw( int speed )
{
	if ( pev->movetype == MOVETYPE_FLY )
	{
		...
		pev->angles.z = UTIL_Approach( target, pev->angles.z, 220.0 * gpGlobalsHL1->frametime );
	}
...
BOOL CFlyingMonster:: ShouldAdvanceRoute( float flWaypointDist )
{
	...
	if ( flWaypointDist <= 64 + (m_flGroundSpeed * gpGlobalsHL1->frametime) )
		return TRUE;
...
void CFlyingMonster::MoveExecute( CBaseEntityHL1 *pTargetEnt, const VectorHL1 &vecDir, float flInterval )
{
	if ( pev->movetype == MOVETYPE_FLY )
	{
		if ( gpGlobalsHL1->time - m_stopTime > 1.0 )
		{
		...
		VectorHL1 vecMove = pev->origin + (( vecDir + (m_vecTravel * m_momentum) ).Normalize() * (m_flGroundSpeed * flInterval));

		if ( m_IdealActivity != m_movementActivity )
		{
			m_flightSpeed = UTIL_Approach( 100, m_flightSpeed, 75 * gpGlobalsHL1->frametime );
			if ( m_flightSpeed < 100 )
				m_stopTime = gpGlobalsHL1->time;
		}
		else
			m_flightSpeed = UTIL_Approach( 20, m_flightSpeed, 300 * gpGlobalsHL1->frametime );
...
float CFlyingMonster::CeilingZ( const VectorHL1 &position )
{
	TraceResult tr;

	VectorHL1 minUp = position;
	VectorHL1 maxUp = position;
	maxUp.z += 4096.0;
...
BOOL CFlyingMonster::ProbeZ( const VectorHL1 &position, const VectorHL1 &probe, float *pFraction)
{
	...
	VectorHL1 ProbeUnit = (probe-position).Normalize();
	...
	while (diff > 1.0)
	{
		...
		VectorHL1 midProbeVec = midProbeLength * ProbeUnit;
...
float CFlyingMonster::FloorZ( const VectorHL1 &position )
{
	TraceResult tr;

	VectorHL1 down = position;
	down.z -= 2048;
