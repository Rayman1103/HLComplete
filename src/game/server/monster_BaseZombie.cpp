...
int g_interactionMonsterZombieMeleeWarning;

envelopePoint_t envDefaultMonsterZombieMoanVolumeFast[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envDefaultMonsterZombieMoanVolume[] =
{
	{	1.0f, 0.1f,
		0.1f, 0.1f,
	},
...
// private activities
//=========================================================
int CMonster_BaseZombie::ACT_ZOM_SWATLEFTMID;
int CMonster_BaseZombie::ACT_ZOM_SWATRIGHTMID;
int CMonster_BaseZombie::ACT_ZOM_SWATLEFTLOW;
int CMonster_BaseZombie::ACT_ZOM_SWATRIGHTLOW;
int CMonster_BaseZombie::ACT_ZOM_RELEASECRAB;
int CMonster_BaseZombie::ACT_ZOM_FALL;

ConVar	sk_monster_zombie_dmg_one_slash( "sk_monster_zombie_dmg_one_slash","0");
ConVar	sk_monster_zombie_dmg_both_slash( "sk_monster_zombie_dmg_both_slash","0");


// When a zombie spawns, he will select a 'base' pitch value
// that's somewhere between basepitchmin & basepitchmax
ConVar monster_zombie_basemin( "monster_zombie_basemin", "100" );
ConVar monster_zombie_basemax( "monster_zombie_basemax", "100" );

ConVar monster_zombie_changemin( "monster_zombie_changemin", "0" );
ConVar monster_zombie_changemax( "monster_zombie_changemax", "0" );

// play a sound once in every monster_zombie_stepfreq steps
ConVar monster_zombie_stepfreq( "monster_zombie_stepfreq", "4" );
ConVar monster_zombie_moanfreq( "monster_zombie_moanfreq", "1" );

ConVar monster_zombie_decaymin( "monster_zombie_decaymin", "0.1" );
ConVar monster_zombie_decaymax( "monster_zombie_decaymax", "0.4" );

ConVar monster_zombie_ambushdist( "monster_zombie_ambushdist", "16000" );

//=========================================================
// For a couple of reasons, we keep a running count of how
// many zombies in the world are angry at any given time.
//=========================================================
static int s_iAngryMonsterZombies = 0;

//=========================================================
//=========================================================
class CAngryMonsterZombieCounter : public CAutoGameSystem
{
public:
	CAngryMonsterZombieCounter( char const *name ) : CAutoGameSystem( name )
	{
	}
	// Level init, shutdown
	virtual void LevelInitPreEntity()
	{
		s_iAngryMonsterZombies = 0;
	}
};

CAngryMonsterZombieCounter	AngryMonsterZombieCounter( "CAngryMonsterZombieCounter" );


int AE_MONSTER_ZOMBIE_ATTACK_RIGHT;
int AE_MONSTER_ZOMBIE_ATTACK_LEFT;
int AE_MONSTER_ZOMBIE_ATTACK_BOTH;
int AE_MONSTER_ZOMBIE_SWATITEM;
int AE_MONSTER_ZOMBIE_STARTSWAT;
int AE_MONSTER_ZOMBIE_STEP_LEFT;
int AE_MONSTER_ZOMBIE_STEP_RIGHT;
int AE_MONSTER_ZOMBIE_SCUFF_LEFT;
int AE_MONSTER_ZOMBIE_SCUFF_RIGHT;
int AE_MONSTER_ZOMBIE_ATTACK_SCREAM;
int AE_MONSTER_ZOMBIE_GET_UP;
int AE_MONSTER_ZOMBIE_POUND;
int AE_MONSTER_ZOMBIE_ALERTSOUND;
int AE_MONSTER_ZOMBIE_POPHEADCRAB;


//=========================================================
//=========================================================
BEGIN_DATADESC( CMonster_BaseZombie )
...
//LINK_ENTITY_TO_CLASS( base_zombie, CMonster_BaseZombie );

//---------------------------------------------------------
//---------------------------------------------------------
int CMonster_BaseZombie::g_numZombies = 0;


//---------------------------------------------------------
//---------------------------------------------------------
CMonster_BaseZombie::CMonster_BaseZombie()
{
...
//---------------------------------------------------------
bool CMonster_BaseZombie::FindNearestPhysicsObject( int iMaxMass )
{
...
//-----------------------------------------------------------------------------
Class_T	CMonster_BaseZombie::Classify( void )
{
	if ( IsSlumped() )
		return CLASS_NONE;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
Disposition_t CMonster_BaseZombie::IRelationType( CBaseEntity *pTarget )
{
...
//-----------------------------------------------------------------------------
float CMonster_BaseZombie::MaxYawSpeed( void )
{
...
// Output :
//-----------------------------------------------------------------------------
bool CMonster_BaseZombie::OverrideMoveFacing( const AILocalMoveGoal_t &move, float flInterval )
{
...
// Output :
//-----------------------------------------------------------------------------
int CMonster_BaseZombie::MeleeAttack1Conditions ( float flDot, float flDist )
{
...
float CMonster_BaseZombie::GetHitgroupDamageMultiplier( int iHitGroup, const CTakeDamageInfo &info )
{
	switch( iHitGroup )
	{
	case HITGROUP_HEAD:
...
// Purpose: 
//-----------------------------------------------------------------------------
void CMonster_BaseZombie::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
...
int CMonster_BaseZombie::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
...
// Output :
//-----------------------------------------------------------------------------
void CMonster_BaseZombie::MakeAISpookySound( float volume, float duration )
{
#ifdef HL2_EPISODIC
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CMonster_BaseZombie::CanPlayMoanSound()
{
	...
	if( s_iAngryMonsterZombies <= 4 )
		return true;

	// This serves to limit the number of zombies that can moan at one time when there are a lot. 
	if( random->RandomInt( 1, monster_zombie_moanfreq.GetInt() * (s_iAngryMonsterZombies/2) ) == 1 )
	{
		return true;
	}
...
//			come through.
//-----------------------------------------------------------------------------
void CMonster_BaseZombie::MoanSound( envelopePoint_t *pEnvelope, int iEnvelopeSize )
{
	if( HasSpawnFlags( SF_NPC_GAG ) )
	{
		// Not yet!
	...
	if( !m_pMoanSound )
	{
		...
		m_flMoanPitch = random->RandomInt( monster_zombie_basemin.GetInt(), monster_zombie_basemax.GetInt() );
	...
	envDefaultMonsterZombieMoanVolumeFast[ 1 ].durationMin = monster_zombie_decaymin.GetFloat();
	envDefaultMonsterZombieMoanVolumeFast[ 1 ].durationMax = monster_zombie_decaymax.GetFloat();
	...
	float flPitch = random->RandomInt( m_flMoanPitch + monster_zombie_changemin.GetInt(), m_flMoanPitch + monster_zombie_changemax.GetInt() );
...
//-----------------------------------------------------------------------------
bool CMonster_BaseZombie::IsChopped( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
bool CMonster_BaseZombie::ShouldIgniteZombieGib( void )
{
#ifdef HL2_EPISODIC
...
//-----------------------------------------------------------------------------
void CMonster_BaseZombie::DieChopped( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
bool CMonster_BaseZombie::ShouldIgnite( const CTakeDamageInfo &info )
{
 	if ( IsOnFire() )
	{
		// Already burning!
...
//-----------------------------------------------------------------------------
void CMonster_BaseZombie::Ignite( float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CMonster_BaseZombie::CopyRenderColorTo( CBaseEntity *pOther )
{
...
//-----------------------------------------------------------------------------
CBaseEntity *CMonster_BaseZombie::ClawAttack( float flDist, int iDamage, QAngle &qaViewPunch, Vector &vecVelocityPunch, int BloodOrigin  )
{
...
// Input  : 
//-----------------------------------------------------------------------------
void CMonster_BaseZombie::PoundSound()
{
	trace_t		tr;
	Vector		forward;
...
// Input  : pEvent - 
//-----------------------------------------------------------------------------
void CMonster_BaseZombie::HandleAnimEvent( animevent_t *pEvent )
{
	if ( pEvent->event == AE_NPC_ATTACK_BROADCAST )
	{
		if( GetEnemy() && GetEnemy()->IsNPC() )
		{
			if( HasCondition(COND_CAN_MELEE_ATTACK1) )
			{
				...
				GetEnemy()->MyNPCPointer()->DispatchInteraction( g_interactionMonsterZombieMeleeWarning, NULL, this );
			}
		}
		return;
	}

	if ( pEvent->event == AE_MONSTER_ZOMBIE_POUND )
	{
		PoundSound();
		return;
	}

	if ( pEvent->event == AE_MONSTER_ZOMBIE_ALERTSOUND )
	{
		AlertSound();
		return;
	}

	if ( pEvent->event == AE_MONSTER_ZOMBIE_STEP_LEFT )
	{
	...
	if ( pEvent->event == AE_MONSTER_ZOMBIE_STEP_RIGHT )
	{
		MakeAIFootstepSound( 180.0f );
		FootstepSound( true );
		return;
	...
	if ( pEvent->event == AE_MONSTER_ZOMBIE_GET_UP )
	{
		...
		if( !IsOnFire() )
		{
			...
			MoanSound( envDefaultMonsterZombieMoanVolumeFast, ARRAYSIZE( envDefaultMonsterZombieMoanVolumeFast ) );
		}
		return;
	}

	if ( pEvent->event == AE_MONSTER_ZOMBIE_SCUFF_LEFT )
	{
		MakeAIFootstepSound( 180.0f );
	...
	if ( pEvent->event == AE_MONSTER_ZOMBIE_SCUFF_RIGHT )
	{
		MakeAIFootstepSound( 180.0f );
		FootscuffSound( true );
		return;
	...
	if ( pEvent->event == AE_MONSTER_ZOMBIE_STARTSWAT )
	{
		MakeAIFootstepSound( 180.0f );
		AttackSound();
		return;
	...
	if ( pEvent->event == AE_MONSTER_ZOMBIE_ATTACK_SCREAM )
	{
		AttackSound();
		return;
	}

	if ( pEvent->event == AE_MONSTER_ZOMBIE_SWATITEM )
	{
	...
	if ( pEvent->event == AE_MONSTER_ZOMBIE_ATTACK_RIGHT )
	{
		...
		ClawAttack( GetClawAttackRange(), sk_monster_zombie_dmg_one_slash.GetFloat(), qa, vec, ZOMBIE_BLOOD_RIGHT_HAND );
		return;
	}

	if ( pEvent->event == AE_MONSTER_ZOMBIE_ATTACK_LEFT )
	{
		Vector right, forward;
		AngleVectors( GetLocalAngles(), &forward, &right, NULL );
		...
		QAngle qa( -15, 20, -10 );
		Vector vec = right + forward;
		ClawAttack( GetClawAttackRange(), sk_monster_zombie_dmg_one_slash.GetFloat(), qa, vec, ZOMBIE_BLOOD_LEFT_HAND );
		return;
	}

	if ( pEvent->event == AE_MONSTER_ZOMBIE_ATTACK_BOTH )
	{
		Vector forward;
		QAngle qaPunch( 45, random->RandomInt(-5,5), random->RandomInt(-5,5) );
		AngleVectors( GetLocalAngles(), &forward );
		forward = forward * 200;
		ClawAttack( GetClawAttackRange(), sk_monster_zombie_dmg_one_slash.GetFloat(), qaPunch, forward, ZOMBIE_BLOOD_BOTH_HANDS );
		return;
	}

	if ( pEvent->event == AE_MONSTER_ZOMBIE_POPHEADCRAB )
	{
		...
		if ( boneIndex == -1 )
		{
			Warning( "AE_MONSTER_ZOMBIE_POPHEADCRAB event using invalid bone name! Usage: event AE_MONSTER_ZOMBIE_POPHEADCRAB \"<BoneName> <Speed>\" \n" );
			return;
		}
		...
		if ( !token )
		{
			Warning( "AE_MONSTER_ZOMBIE_POPHEADCRAB event format missing velocity parameter! Usage: event AE_MONSTER_ZOMBIE_POPHEADCRAB \"<BoneName> <Speed>\" \n" );
			return;
		}
...
//-----------------------------------------------------------------------------
void CMonster_BaseZombie::Spawn( void )
{
	SetSolid( SOLID_BBOX );
...
//-----------------------------------------------------------------------------
void CMonster_BaseZombie::Precache( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CMonster_BaseZombie::StartTouch( CBaseEntity *pOther )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CMonster_BaseZombie::CreateBehaviors()
{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CMonster_BaseZombie::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
...
//-----------------------------------------------------------------------------
void CMonster_BaseZombie::BuildScheduleTestBits( void )
{
...
//-----------------------------------------------------------------------------
void CMonster_BaseZombie::OnScheduleChange( void )
{
	//
...
//---------------------------------------------------------
//---------------------------------------------------------
int	CMonster_BaseZombie::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CMonster_BaseZombie::SelectSchedule ( void )
{
	...
	case NPC_STATE_COMBAT:
		if ( HasCondition( COND_NEW_ENEMY ) && GetEnemy() )
		{
			...
			if (flDist >= monster_zombie_ambushdist.GetFloat() && MustCloseToAttack() )
			{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CMonster_BaseZombie::IsSlumped( void )
{
	if( hl2_episodic.GetBool() )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CMonster_BaseZombie::IsGettingUp( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CMonster_BaseZombie::GetSwatActivity( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CMonster_BaseZombie::GatherConditions( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CMonster_BaseZombie::PrescheduleThink( void )
{
	BaseClass::PrescheduleThink();
	
#if 0
	DevMsg(" ** %d Angry Zombies **\n", s_iAngryMonsterZombies );
#endif
...
//---------------------------------------------------------
//---------------------------------------------------------
void CMonster_BaseZombie::StartTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CMonster_BaseZombie::RunTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_ZOMBIE_SWAT_ITEM:
...
//---------------------------------------------------------
//---------------------------------------------------------
void CMonster_BaseZombie::Event_Killed( const CTakeDamageInfo &info )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CMonster_BaseZombie::BecomeRagdoll( const CTakeDamageInfo &info, const Vector &forceVector )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CMonster_BaseZombie::StopLoopingSounds()
{
...
bool CMonster_BaseZombie::ShouldPlayFootstepMoan( void )
{
	if( random->RandomInt( 1, monster_zombie_stepfreq.GetInt() * s_iAngryMonsterZombies ) == 1 )
	{
		return true;
	}
...
// as well.
//---------------------------------------------------------
float CMonster_BaseZombie::DistToPhysicsEnt( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMonster_BaseZombie::OnStateChange( NPC_STATE OldState, NPC_STATE NewState )
{
	switch( NewState )
	{
	case NPC_STATE_COMBAT:
...
//-----------------------------------------------------------------------------
Activity CMonster_BaseZombie::NPC_TranslateActivity( Activity baseAct )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
Vector CMonster_BaseZombie::BodyTarget( const Vector &posSrc, bool bNoisy ) 
{ 
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
Vector CMonster_BaseZombie::HeadTarget( const Vector &posSrc )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CMonster_BaseZombie::GetAutoAimRadius()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CMonster_BaseZombie::OnInsufficientStopDist( AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult )
{
...
//			&chasePosition - 
//-----------------------------------------------------------------------------
void CMonster_BaseZombie::TranslateNavGoal( CBaseEntity *pEnemy, Vector &chasePosition )
{
...
AI_BEGIN_CUSTOM_NPC( base_zombie, CMonster_BaseZombie )
	...
	DECLARE_ANIMEVENT( AE_MONSTER_ZOMBIE_ATTACK_RIGHT )
	DECLARE_ANIMEVENT( AE_MONSTER_ZOMBIE_ATTACK_LEFT )
	DECLARE_ANIMEVENT( AE_MONSTER_ZOMBIE_ATTACK_BOTH )
	DECLARE_ANIMEVENT( AE_MONSTER_ZOMBIE_SWATITEM )
	DECLARE_ANIMEVENT( AE_MONSTER_ZOMBIE_STARTSWAT )
	DECLARE_ANIMEVENT( AE_MONSTER_ZOMBIE_STEP_LEFT )
	DECLARE_ANIMEVENT( AE_MONSTER_ZOMBIE_STEP_RIGHT )
	DECLARE_ANIMEVENT( AE_MONSTER_ZOMBIE_SCUFF_LEFT )
	DECLARE_ANIMEVENT( AE_MONSTER_ZOMBIE_SCUFF_RIGHT )
	DECLARE_ANIMEVENT( AE_MONSTER_ZOMBIE_ATTACK_SCREAM )
	DECLARE_ANIMEVENT( AE_MONSTER_ZOMBIE_GET_UP )
	DECLARE_ANIMEVENT( AE_MONSTER_ZOMBIE_POUND )
	DECLARE_ANIMEVENT( AE_MONSTER_ZOMBIE_ALERTSOUND )
	DECLARE_ANIMEVENT( AE_MONSTER_ZOMBIE_POPHEADCRAB )

	DECLARE_INTERACTION( g_interactionMonsterZombieMeleeWarning )
