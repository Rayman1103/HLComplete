...
VectorHL1 VecBModelOrigin( entvars_t* pevBModel );
...
int CBaseMonster::SaveHL1( CSaveHL1 &save )
{
	if ( !CBaseToggle::SaveHL1(save) )
		return 0;
	return save.WriteFields( "CBaseMonster", this, m_SaveData, ARRAYSIZE(m_SaveData) );
}

int CBaseMonster::RestoreHL1( CRestoreHL1 &restore )
{
	if ( !CBaseToggle::RestoreHL1(restore) )
		return 0;
...
{
	m_flHungryTime = gpGlobalsHL1->time + flFullDuration;
}
...
{
	if ( m_flHungryTime > gpGlobalsHL1->time )
	{
		return FALSE;
	}
...
void CBaseMonster :: Listen ( void )
{
	...
	iSound = CSoundEntHL1::ActiveList();
	...
	{
		pCurrentSound = CSoundEntHL1::SoundPointerForIndex( iSound );
...
// volume of the sound. 
//=========================================================
float CBaseMonster :: FLSoundVolume ( CSoundHL1 *pSound )
{
...
void CBaseMonster :: Look ( int iDistance )
{
	...
	CBaseEntityHL1	*pSightEnt = NULL;// the current visible entity that we're dealing with

	// See no evil if prisoner is set
	if ( !FBitSet( pev->spawnflags, SF_MONSTER_PRISONER ) )
	{
		CBaseEntityHL1 *pList[100];

		VectorHL1 delta = VectorHL1( iDistance, iDistance, iDistance );
...
//=========================================================
CSoundHL1* CBaseMonster :: PBestSound ( void )
{	
	int iThisSound; 
	int	iBestSound = -1;
	...
	{
		pSound = CSoundEntHL1::SoundPointerForIndex( iThisSound );
	...
	if ( iBestSound >= 0 )
	{
		pSound = CSoundEntHL1::SoundPointerForIndex( iBestSound );
		return pSound;
	}
#if _DEBUG
	ALERT( at_error, "NULL Return from PBestSound\n" );
...
//=========================================================
CSoundHL1* CBaseMonster :: PBestScent ( void )
{	
	int iThisScent; 
	int	iBestScent = -1;
	...
	{
		pSound = CSoundEntHL1::SoundPointerForIndex( iThisScent );
	...
	if ( iBestScent >= 0 )
	{
		pSound = CSoundEntHL1::SoundPointerForIndex( iBestScent );
...
{
	pev->nextthink = gpGlobalsHL1->time + 0.1;// keep monster thinking.
...
BOOL CBaseMonster::MoveToLocation( Activity movementAct, float waitTime, const VectorHL1 &goal )
{
	m_movementActivity = movementAct;
	m_moveWaitTime = waitTime;
...
BOOL CBaseMonster::MoveToNode( Activity movementAct, float waitTime, const VectorHL1 &goal )
{
	m_movementActivity = movementAct;
	m_moveWaitTime = waitTime;
...
void CBaseMonster :: CheckAttacks ( CBaseEntityHL1 *pTarget, float flDist )
{
	...
	flDot = DotProduct (vec2LOS , gpGlobalsHL1->v_forward.Make2D() );
...
//=========================================================
int CBaseMonster :: CheckEnemy ( CBaseEntityHL1 *pEnemy )
{
	float	flDistToEnemy;
	...
	VectorHL1 vecEnemyPos = pEnemy->pev->origin;
	...
	if ( HasConditions( bits_COND_SEE_ENEMY ) )
	{
		...
		if (pEnemy->pev->velocity != VectorHL1( 0, 0, 0))
		{
...
int CBaseMonster :: CheckLocalMove ( const VectorHL1 &vecStart, const VectorHL1 &vecEnd, CBaseEntityHL1 *pTarget, float *pflDist )
{
	VectorHL1	vecStartPos;// record monster's position before trying the move
	float	flYaw;
	float	flDist;
...
//		UTIL_ParticleEffect ( pev->origin, g_vecZero, 255, 25 );
		...
		{// can't take the next step, fail!
			...
			if ( pTarget && pTarget->edict() == gpGlobalsHL1->trace_ent )
			{
...
float CBaseMonster :: OpenDoorAndWait( entvars_t *pevDoor )
{
	...
	CBaseEntityHL1 *pcbeDoor = CBaseEntityHL1::Instance(pevDoor);
		...
		if ( pcbeDoor->pev->targetname )
		{
			...
			for (;;)
			{
				...
				if ( VARS( pentTarget ) != pcbeDoor->pev )
				{
					...
					{
						CBaseEntityHL1 *pDoor = Instance(pentTarget);
						if ( pDoor )
	...
	return gpGlobalsHL1->time + flTravelTime;
}
...
//							ALERT( at_aiconsole, "Wating for door %.2f\n", m_flMoveWaitFinished-gpGlobalsHL1->time );
						}
					}
				}
				//ALERT(at_aiconsole, "\n");
...
// BuildRoute
//=========================================================
BOOL CBaseMonster :: BuildRoute ( const VectorHL1 &vecGoal, int iMoveFlag, CBaseEntityHL1 *pTarget )
{
	float	flDist;
	VectorHL1	vecApex;
	int		iLocalMove;
...
//=========================================================
BOOL CBaseMonster :: FTriangulate ( const VectorHL1 &vecStart , const VectorHL1 &vecEnd, float flDist, CBaseEntityHL1 *pTargetEnt, VectorHL1 *pApex )
{
	VectorHL1		vecDir;
	VectorHL1		vecForward;
	VectorHL1		vecLeft;// the spot we'll try to triangulate to on the left
	VectorHL1		vecRight;// the spot we'll try to triangulate to on the right
	VectorHL1		vecTop;// the spot we'll try to triangulate to on the top
	VectorHL1		vecBottom;// the spot we'll try to triangulate to on the bottom
	VectorHL1		vecFarSide;// the spot that we'll move to after hitting the triangulated point, before moving on to our normal goal.
	int			i;
	float		sizeX, sizeZ;
...
void CBaseMonster :: Move ( float flInterval ) 
{
	...
	if ( m_flMoveWaitFinished > gpGlobalsHL1->time )
		return;
...
//	DrawRoute( pev, m_Route, m_iRouteIndex, 0, 200, 0 );
	...
	{
		CBaseEntityHL1 *pBlocker;

		// Can't move, stop
		Stop();
		// Blocking entity is in global trace_ent
		pBlocker = CBaseEntityHL1::Instance( gpGlobalsHL1->trace_ent );
		if (pBlocker)
		{
			DispatchBlocked( edict(), pBlocker->edict() );
		}

		if ( pBlocker && m_moveWaitTime > 0 && pBlocker->IsMoving() && !pBlocker->IsPlayer() && (gpGlobalsHL1->time-m_flMoveWaitFinished) > 3.0 )
		{
			// Can we still move toward our target?
			if ( flDist < m_flGroundSpeed )
			{
				// No, Wait for a second
				m_flMoveWaitFinished = gpGlobalsHL1->time + m_moveWaitTime;
				return;
			}
...
//				ALERT ( at_aiconsole, "Couldn't Triangulate\n" );
				...
				if ( m_moveWaitTime > 0 && !(m_afMemory & bits_MEMORY_MOVE_FAILED) )
				{
					...
					{
						// Don't get stuck
						if ( (gpGlobalsHL1->time - m_flMoveWaitFinished) < 0.2 )
							Remember( bits_MEMORY_MOVE_FAILED );

						m_flMoveWaitFinished = gpGlobalsHL1->time + 0.1;
					}
				}
				else
				{
...
//		REMOVE_ENTITY(ENT(pev));
	...
	SetThinkHL1( MonsterInitThink );
	pev->nextthink = gpGlobalsHL1->time + 0.1;
	SetUseHL1 ( MonsterUse );
}
...
void CBaseMonster :: StartMonster ( void )
{
	...
	if ( !FStringNull(pev->target) )// this monster has a target
	{
		...
		m_pGoalEnt = CBaseEntityHL1::Instance( FIND_ENTITY_BY_TARGETNAME ( NULL, STRING( pev->target ) ) );
...
//=========================================================
int CBaseMonster::IRelationship ( CBaseEntityHL1 *pTarget )
{
...
BOOL CBaseMonster :: FindCover ( VectorHL1 vecThreat, VectorHL1 vecViewOffset, float flMinDist, float flMaxDist )
{
	int i;
	int iMyHullIndex;
	int iMyNode;
...
// value
//=========================================================
BOOL CBaseMonster :: BuildNearestRoute ( VectorHL1 vecThreat, VectorHL1 vecViewOffset, float flMinDist, float flMaxDist )
{
	int i;
	int iMyHullIndex;
	int iMyNode;
...
//=========================================================
CBaseEntityHL1 *CBaseMonster :: BestVisibleEnemy ( void )
{
	CBaseEntityHL1	*pReturn;
	CBaseEntityHL1	*pNextEnt;
	int			iNearest;
	int			iDist;
...
float CBaseMonster::ChangeYaw ( int yawSpeed )
{
	...
	if (current != ideal)
	{
		speed = (float)yawSpeed * gpGlobalsHL1->frametime * 10;
		move = ideal - current;
...
//=========================================================
float	CBaseMonster::VecToYaw ( VectorHL1 vecDir )
{
...
VectorHL1 CBaseMonster :: GetGunPosition( )
{
	UTIL_MakeVectors(pev->angles);

	// VectorHL1 vecSrc = pev->origin + gpGlobalsHL1->v_forward * 10;
	//vecSrc.z = pevShooter->absmin.z + pevShooter->size.z * 0.7;
	//vecSrc.z = pev->origin.z + (pev->view_ofs.z - 4);
	VectorHL1 vecSrc = pev->origin 
					+ gpGlobalsHL1->v_forward * m_HackedGunPos.y 
					+ gpGlobalsHL1->v_right * m_HackedGunPos.x 
					+ gpGlobalsHL1->v_up * m_HackedGunPos.z;
...
// exists )
//=========================================================
BOOL CBaseMonster :: FGetNodeRoute ( VectorHL1 vecDest )
{
...
void CBaseMonster::ReportAIState( void )
{
	...
	if ( IsMoving() )
	{
		ALERT( level, " Moving " );
		if ( m_flMoveWaitFinished > gpGlobalsHL1->time )
			ALERT( level, ": Stopped for %.2f. ", m_flMoveWaitFinished - gpGlobalsHL1->time );
...
BOOL CBaseMonster :: FindLateralCover ( const VectorHL1 &vecThreat, const VectorHL1 &vecViewOffset )
{
	TraceResult	tr;
	VectorHL1	vecBestOnLeft;
	VectorHL1	vecBestOnRight;
	VectorHL1	vecLeftTest;
	VectorHL1	vecRightTest;
	VectorHL1	vecStepRight;
	int		i;

	UTIL_MakeVectors ( pev->angles );
	vecStepRight = gpGlobalsHL1->v_right * COVER_DELTA;
	vecStepRight.z = 0; 
...
VectorHL1 CBaseMonster :: ShootAtEnemy( const VectorHL1 &shootOrigin )
{
	CBaseEntityHL1 *pEnemy = m_hEnemy;
...
BOOL CBaseMonster :: BBoxFlat ( void )
{
	...
	UTIL_TraceLine ( vecPoint, vecPoint - VectorHL1 ( 0, 0, 100 ), ignore_monsters, ENT(pev), &tr );
	...
	if ( HasConditions(bits_COND_SEE_HATE | bits_COND_SEE_DISLIKE | bits_COND_SEE_NEMESIS) )
	{
		...
		if ( pNewEnemy != m_hEnemy && pNewEnemy != NULL)
		{
			...
			if ( m_pSchedule )
			{
				...
				{
					CBaseEntityHL1 *pOwner = GetMonsterPointer( pNewEnemy->pev->owner );
...
//=========================================================
CBaseEntityHL1* CBaseMonster :: DropItem ( char *pszItemName, const VectorHL1 &vecPos, const VectorHL1 &vecAng )
{
	if ( !pszItemName )
	{
	...
	CBaseEntityHL1 *pItem = CBaseEntityHL1::Create( pszItemName, vecPos, vecAng, edict() );

	if ( pItem )
	{
		// do we want this behavior to be default?! (sjb)
		pItem->pev->velocity = pev->velocity;
		pItem->pev->avelocity = VectorHL1 ( 0, RANDOM_FLOAT( 0, 100 ), 0 );
		return pItem;
	}
	else
	{
