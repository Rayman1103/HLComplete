...
extern void UTIL_MoveToOrigin( edict_t_hl1* pent, const VectorHL1 &vecGoal, float flDist, int iMoveType ); 

VectorHL1 VecCheckToss ( entvars_t *pev, const VectorHL1 &vecSpot1, VectorHL1 vecSpot2, float flGravityAdj = 1.0 );
VectorHL1 VecCheckThrow ( entvars_t *pev, const VectorHL1 &vecSpot1, VectorHL1 vecSpot2, float flSpeed, float flGravityAdj = 1.0 );
extern DLL_GLOBAL VectorHL1		g_vecAttackDirHL1;
...
extern void EjectBrass (const VectorHL1 &vecOrigin, const VectorHL1 &vecVelocity, float rotation, int model, int soundtype );
extern void ExplodeModel( const VectorHL1 &vecOrigin, float speed, int model, int count );

BOOL FBoxVisible ( entvars_t *pevLooker, entvars_t *pevTarget );
BOOL FBoxVisible ( entvars_t *pevLooker, entvars_t *pevTarget, VectorHL1 &vecTargetOrigin, float flSize = 0.0 );
...
//
class CGib : public CBaseEntityHL1
{
public:
	void Spawn( const char *szGibModel );
	void EXPORT BounceGibTouch ( CBaseEntityHL1 *pOther );
	void EXPORT StickyGibTouch ( CBaseEntityHL1 *pOther );
	void EXPORT WaitTillLand( void );
	void		LimitVelocity( void );

	virtual int	ObjectCaps( void ) { return (CBaseEntityHL1 :: ObjectCaps() & ~FCAP_ACROSS_TRANSITION) | FCAP_DONT_SAVE; }
	static	void SpawnHeadGib( entvars_t *pevVictim );
	static	void SpawnRandomGibs( entvars_t *pevVictim, int cGibs, int human );
	static  void SpawnStickyGibs( entvars_t *pevVictim, VectorHL1 vecOrigin, int cGibs );
