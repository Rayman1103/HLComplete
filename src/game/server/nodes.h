...
{
public:
	VectorHL1	m_vecOrigin;// location of this node in space
	VectorHL1  m_vecOriginPeek; // location of this node (LAND nodes are NODE_HEIGHT higher).
...
#define NUM_RANGES 256
	...
	int		FindNearestNode ( const VectorHL1 &vecOrigin, CBaseEntityHL1 *pEntity );
	int		FindNearestNode ( const VectorHL1 &vecOrigin, int afNodeTypes );
	//int		FindNearestLink ( const VectorHL1 &vecTestPoint, int *piNearestLink, BOOL *pfAlongLine );
	...
	void	CheckNode(VectorHL1 vecOrigin, int iNode);
	...
	int			HullIndex( const CBaseEntityHL1 *pEntity );	// what hull the monster uses
	int			NodeType( const CBaseEntityHL1 *pEntity );		// what node type the monster uses
...
//=========================================================
class CNodeEnt : public CBaseEntityHL1
{
	void Spawn( void );
