...
#define PLAYER_FATAL_FALL_SPEED_HL1		1024// approx 60 feet
#define PLAYER_MAX_SAFE_FALL_SPEED_HL1	580// approx 20 feet
#define DAMAGE_FOR_FALL_SPEED_HL1		(float) 100 / ( PLAYER_FATAL_FALL_SPEED - PLAYER_MAX_SAFE_FALL_SPEED )// damage per unit per second.
#define PLAYER_MIN_BOUNCE_SPEED_HL1		200
...
class CBasePlayerHL1 : public CBaseMonster
{
public:
	...
	edict_t_hl1		   *m_pentSndLast;			// last sound entity to modify player room type
...
// these are time-sensitive things that we keep track of
	...
	EHANDLEHL1			m_pTank;				// the tank which the player is currently controlling,  NULL if no tank
	...
	// usable player items 
	CBasePlayerItemHL1	*m_rgpPlayerItems[MAX_ITEM_TYPES];
	CBasePlayerItemHL1 *m_pActiveItem;
	CBasePlayerItemHL1 *m_pClientActiveItem;  // client version of the active item
	CBasePlayerItemHL1 *m_pLastItem;
	// shared ammo slots
	int	m_rgAmmo[MAX_AMMO_SLOTS];
	int	m_rgAmmoLast[MAX_AMMO_SLOTS];

	VectorHL1			m_vecAutoAim;
	BOOL				m_fOnTarget;
	int					m_iDeaths;
	float				m_iRespawnFrames;	// used in PlayerDeathThink() to make sure players can always respawn
...
//	virtual void Think( void );
	...
	virtual int TakeHealth( float flHealth, int bitsDamageType );
	virtual void TraceAttackHL1( entvars_t *pevAttacker, float flDamage, VectorHL1 vecDir, TraceResult *ptr, int bitsDamageType);
	virtual int TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType);
	virtual void	Killed( entvars_t *pevAttacker, int iGib );
	virtual VectorHL1 BodyTarget( const VectorHL1 &posSrc ) { return Center( ) + pev->view_ofs * RANDOM_FLOAT( 0.5, 1.1 ); };		// position to shoot at
	virtual void StartSneaking( void ) { m_tSneaking = gpGlobalsHL1->time - 1; }
	virtual void StopSneaking( void ) { m_tSneaking = gpGlobalsHL1->time + 30; }
	virtual BOOL IsSneaking( void ) { return m_tSneaking <= gpGlobalsHL1->time; }
	...
	BOOL SwitchWeapon( CBasePlayerItemHL1 *pWeapon );
	...
	// custom player functions
	virtual void ImpulseCommands( void );
	void CheatImpulseCommandsHL1( int iImpulse );

	void StartDeathCam( void );
	void StartObserver( Vector vecPosition, VectorHL1 vecViewAngle );

	void AddPoints( int score, BOOL bAllowNegativeScore );
	void AddPointsToTeam( int score, BOOL bAllowNegativeScore );
	BOOL AddPlayerItem( CBasePlayerItemHL1 *pItem );
	BOOL RemovePlayerItem( CBasePlayerItemHL1 *pItem );
	void DropPlayerItem ( char *pszItemName );
	BOOL HasPlayerItem( CBasePlayerItemHL1 *pCheckItem );
	...
	void ResetAutoaim( void );
	VectorHL1 GetAutoaimVector( float flDelta  );
	VectorHL1 AutoaimDeflection( VectorHL1 &vecSrc, float flDist, float flDelta  );
	...
	void CBasePlayerHL1::TabulateAmmo( void );
