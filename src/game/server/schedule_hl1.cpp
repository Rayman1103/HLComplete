...
//					pev->solid = SOLID_NOT;
					UTIL_SetSize ( pev, VectorHL1 ( -4, -4, 0 ), VectorHL1 ( 4, 4, 1 ) );
				}
				else // !!!HACKHACK - put monster in a thin, wide bounding box until we fix the solid type/bounding volume problem
					UTIL_SetSize ( pev, VectorHL1 ( pev->mins.x, pev->mins.y, pev->mins.z ), VectorHL1 ( pev->maxs.x, pev->maxs.y, pev->mins.z + 1 ) );
...
void CBaseMonster :: StartTask ( Task_t_hl1 *pTask )
{
	...
	case TASK_PLAY_SEQUENCE:
		{
			m_IdealActivity = ( ActivityHL1 )( int )pTask->flData;
			break;
		}
	case TASK_PLAY_ACTIVE_IDLE:
		{
			...
			m_IdealActivity = ( ActivityHL1 )WorldGraph.m_pNodes[ m_iHintNode ].m_sHintActivity;
			break;
		}
