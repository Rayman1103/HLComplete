...
void CCineMonster :: Spawn( void )
{
	...
	{
		SetThinkHL1( CineThink );
		pev->nextthink = gpGlobalsHL1->time + 1.0;
		// Wait to be used?
		if ( pev->targetname )
			m_startTime = gpGlobalsHL1->time + 1E6;
	}
...
// CineStart
//
void CCineMonster :: Use( CBaseEntityHL1 *pActivator, CBaseEntityHL1 *pCaller, USE_TYPE useType, float value )
{
	// do I already know who I should use
	CBaseEntityHL1	*pEntity = m_hTargetEnt;
	...
	if ( pTarget )
	{
		...
		m_startTime = gpGlobalsHL1->time + 0.05;
	}
	else
	{
		// if not, try finding them
...
void CCineMonster :: Blocked( CBaseEntityHL1 *pOther )
{

}

void CCineMonster :: Touch( CBaseEntityHL1 *pOther )
{
/*
...
int CCineMonster :: FindEntity( void )
{
	...
	pentTarget = FIND_ENTITY_BY_TARGETNAME(NULL, STRING_HL1(m_iszEntity));
	m_hTargetEnt = NULL;
	CBaseMonster	*pTarget = NULL;
	...
	while (!FNullEnt(pentTarget))
	{
		if ( FBitSet( VARS(pentTarget)->flags, FL_MONSTER ))
		{
			...
			ALERT( at_console, "Found %s, but can't play!\n", STRING_HL1(m_iszEntity) );
		}
		pentTarget = FIND_ENTITY_BY_TARGETNAME(pentTarget, STRING_HL1(m_iszEntity));
		pTarget = NULL;
	}
	...
	if ( !pTarget )
	{
		...
		{
			if (FClassnameIs( pEntity->pev, STRING_HL1(m_iszEntity)))
			{
...
//		ALERT( at_aiconsole, "\"%s\" found and used (INT: %s)\n", STRING( pTarget->pev->targetname ), FBitSet(pev->spawnflags, SF_SCRIPT_NOINTERRUPT)?"No":"Yes" );
		...
		if (m_iszIdle)
		{
			...
			if (FStrEq( STRING_HL1(m_iszIdle), STRING_HL1(m_iszPlay)))
			{
...
void CCineMonster :: CineThink( void )
{
	...
	{
		PossessEntity( );
		ALERT( at_aiconsole, "script \"%s\" using monster \"%s\"\n", STRING( pev->targetname ), STRING_HL1( m_iszEntity ) );
	}
	else
	{
		CancelScript( );
...
BOOL CCineMonster :: StartSequence( CBaseMonster *pTarget, int iszSeq, BOOL completeOnEmpty )
{
	...
	pTarget->pev->sequence = pTarget->LookupSequence( STRING_HL1( iszSeq ) );
	if (pTarget->pev->sequence == -1)
	{
		ALERT( at_error, "%s: unknown scripted sequence \"%s\"\n", STRING( pTarget->pev->targetname ), STRING_HL1( iszSeq) );
...
void ScriptEntityCancel( edict_t *pentCine )
{
	...
	if (FClassnameIs( pentCine, CLASSNAME ))
	{
		...
		CBaseEntityHL1	*pEntity = pCineTarget->m_hTargetEnt;
		CBaseMonster	*pTarget = NULL;
		if ( pEntity )
			pTarget = pEntity->MyMonsterPointer();
...
void CCineMonster :: Activate( void )
{
	...
	if ( !pTarget )
	{
		pentTarget = FIND_ENTITY_BY_CLASSNAME(NULL, STRING_HL1(m_iszEntity));
		while (!pTarget && !FNullEnt(pentTarget))
		{
			pTarget = GetMonsterPointer( pentTarget );
			pentTarget = FIND_ENTITY_BY_TARGETNAME(pentTarget, STRING_HL1(m_iszEntity));
	...
	if ( pTarget )
	{
		...
		if ( pmodel )
		{
			...
			SequencePrecache( pmodel, STRING_HL1( m_iszIdle ) );
			SequencePrecache( pmodel, STRING_HL1( m_iszPlay ) );
		}
	}
}
...
BOOL CBaseMonster :: CineCleanup( )
{
	...
	if (pev->deadflag == DEAD_DYING)
	{
		...
		UTIL_SetSizeHL1( pev, pev->mins, Vector(pev->maxs.x, pev->maxs.y, pev->mins.z + 2) );

		if ( pOldCine && FBitSet( pOldCine->pev->spawnflags, SF_SCRIPT_LEAVECORPSE ) )
		{
			SetUseHL1( NULL );		// BUGBUG -- This doesn't call Killed()
			SetThinkHL1( NULL );	// This will probably break some stuff
			SetTouchHL1( NULL );
		}
		else
...
class CScriptedSentence : public CBaseToggleHL1
{
public:
	void Spawn( void );
	...
	int	 ObjectCaps( void ) { return (CBaseToggleHL1 :: ObjectCaps() & ~FCAP_ACROSS_TRANSITION); }
...
IMPLEMENT_SAVERESTORE( CScriptedSentence, CBaseToggleHL1 );
...
void CScriptedSentence :: KeyValue( KeyValueData *pkvd )
{
	...
	else if (FStrEq(pkvd->szKeyName, "listener"))
	{
		...
		CBaseToggleHL1::KeyValue( pkvd );
}


void CScriptedSentence :: Use( CBaseEntityHL1 *pActivator, CBaseEntityHL1 *pCaller, USE_TYPE useType, float value )
{
	if ( !m_active )
		return;
//	ALERT( at_console, "Firing sentence: %s\n", STRING(m_iszSentence) );
	SetThinkHL1( FindThink );
	pev->nextthink = gpGlobalsHL1->time;
}
...
void CScriptedSentence :: FindThink( void )
{
	...
	if ( pMonster )
	{
		...
		if ( pev->spawnflags & SF_SENTENCE_ONCE )
			UTIL_Remove( this );
		SetThinkHL1( DelayThink );
		pev->nextthink = gpGlobalsHL1->time + m_flDuration + m_flRepeat;
		m_active = FALSE;
...
//		ALERT( at_console, "%s: can't find monster %s\n", STRING(m_iszSentence), STRING(m_iszEntity) );
		pev->nextthink = gpGlobalsHL1->time + m_flRepeat + 0.5;
	}
}
...
BOOL CScriptedSentence :: StartSentence( CBaseMonster *pTarget )
{
	...
	CBaseEntityHL1 *pListener = NULL;
	if (!FStringNull(m_iszListener))
	{
		float radius = m_flRadius;

		if ( FStrEq( STRING_HL1(m_iszListener ), "player" ) )
			radius = 4096;	// Always find the player

		pListener = UTIL_FindEntityGeneric( STRING_HL1( m_iszListener ), pTarget->pev->origin, radius );
	}

	pTarget->PlayScriptedSentence( STRING_HL1(m_iszSentence), m_flDuration,  m_flVolume, m_flAttenuation, bConcurrent, pListener );
	ALERT( at_aiconsole, "Playing sentence %s (%.1f)\n", STRING_HL1(m_iszSentence), m_flDuration );
	SUB_UseTargets( NULL, USE_TOGGLE, 0 );
	return TRUE;
}
