...
IMPLEMENT_SAVERESTORE( CEnvSound, CBaseEntityHL1 );
...
void CEnvSound :: Think( void )
{
	...
	pPlayer = GetClassPtr( (CBasePlayerHL1 *)VARS(pentPlayer));
	float flRange;
...
env_sound_Think_fast:
	pev->nextthink = gpGlobalsHL1->time + 0.25;
	return;

env_sound_Think_slow:
	pev->nextthink = gpGlobalsHL1->time + 0.75;
	return;
}
...
{
	// spread think times
	pev->nextthink = gpGlobalsHL1->time + RANDOM_FLOAT(0.0, 0.5); 
}
