...
{
	DEFINE_FIELD_HL1( CSquadMonster, m_hSquadLeader, FIELD_EHANDLE_HL1 ),
	DEFINE_ARRAY_HL1( CSquadMonster, m_hSquadMember, FIELD_EHANDLE_HL1, MAX_SQUAD_MEMBERS - 1 ),

	// DEFINE_FIELD_HL1( CSquadMonster, m_afSquadSlots, FIELD_INTEGER_HL1 ), // these need to be reset after transitions!
	DEFINE_FIELD_HL1( CSquadMonster, m_fEnemyEluded, FIELD_BOOLEAN_HL1 ),
	DEFINE_FIELD_HL1( CSquadMonster, m_flLastEnemySightTime, FIELD_TIME_HL1 ),

	DEFINE_FIELD_HL1( CSquadMonster, m_iMySlot, FIELD_INTEGER_HL1 ),
...
//
//=========================================================
void CSquadMonster :: SquadMakeEnemy ( CBaseEntityHL1 *pEnemy )
{
	if (!InSquad())
		return;
...
int CSquadMonster :: SquadRecruit( int searchRadius, int maxMembers )
{
	...
	if ( !FStringNull( pev->netname ) )
	{
		...
		pEntity = UTIL_FindEntityByString( pEntity, "netname", STRING_HL1( pev->netname ) );
		while ( pEntity )
		{
		...
		while ((pEntity = UTIL_FindEntityInSphere( pEntity, pev->origin, searchRadius )) != NULL)
		{
			...
			if ( pRecruit && pRecruit != this && pRecruit->IsAlive() && !pRecruit->m_pCine )
			{
				...
				if ( !pRecruit->InSquad() && pRecruit->Classify() == iMyClass &&
				   ( (iMyClass != CLASS_ALIEN_MONSTER) || FStrEq(STRING_HL1(pev->classname), STRING_HL1(pRecruit->pev->classname))) &&
...
// CheckEnemy
//=========================================================
int CSquadMonster :: CheckEnemy ( CBaseEntityHL1 *pEnemy )
{
	int iUpdatedLKP;
	...
	if ( InSquad() && (CBaseEntityHL1 *)m_hEnemy == MySquadLeader()->m_hEnemy )
	{
		if ( iUpdatedLKP )
		{
...
BOOL CSquadMonster :: NoFriendlyFire( void )
{
	...
	VectorHL1	vecLeftSide;
	VectorHL1	vecRightSide;
	VectorHL1	v_left;
	...
	vecLeftSide = pev->origin - ( gpGlobalsHL1->v_right * ( pev->size.x * 1.5 ) );
	vecRightSide = pev->origin + ( gpGlobalsHL1->v_right * ( pev->size.x * 1.5 ) );
	v_left = gpGlobalsHL1->v_right * -1;

	leftPlane.InitializePlane ( gpGlobalsHL1->v_right, vecLeftSide );
	rightPlane.InitializePlane ( v_left, vecRightSide );
	backPlane.InitializePlane ( gpGlobalsHL1->v_forward, pev->origin );
...
//=========================================================
BOOL CSquadMonster :: FValidateCover ( const VectorHL1 &vecCoverLocation )
{
	if ( !InSquad() )
	{
		return TRUE;
...
BOOL CSquadMonster :: SquadEnemySplit ( void )
{
	...
	CSquadMonster	*pSquadLeader = MySquadLeader();
	CBaseEntityHL1		*pEnemy	= pSquadLeader->m_hEnemy;
...
// on proximity to others in the squad)
//=========================================================
BOOL CSquadMonster :: SquadMemberInRange ( const VectorHL1 &vecLocation, float flDist )
{
	if (!InSquad())
		return FALSE;
