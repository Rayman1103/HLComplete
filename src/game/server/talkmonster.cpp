...
Task_t_hl1	tlTlkIdleWatchClient[] =
{
	{ TASK_STOP_MOVING,			0				},
...
Task_t_hl1	tlTlkIdleWatchClientStare[] =
{
	{ TASK_STOP_MOVING,			0				},
	{ TASK_SET_ACTIVITY,		(float)ACT_IDLE	},
...
Task_t_hl1	tlTlkIdleEyecontact[] =
{
	{ TASK_TLK_IDEALYAW,	(float)0		},// look at who I'm talking to
	{ TASK_FACE_IDEAL,		(float)0		}, 
	{ TASK_SET_ACTIVITY,	(float)ACT_SIGNAL3	},
...
void CTalkMonster :: SetActivity ( ActivityHL1 newActivity )
{
...
void CTalkMonster :: StartTask( Task_t_hl1 *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_TLK_SPEAK:
	...
	case TASK_TLK_CLIENT_STARE:
		...
		m_flWaitFinished = gpGlobalsHL1->time + pTask->flData;
		break;
	...
	case TASK_MOVE_AWAY_PATH:
		{
			...
			else if ( FindCover( pev->origin, pev->view_ofs, 0, CoverRadius() ) )
			{
				...
				m_flMoveWaitFinished = gpGlobalsHL1->time + 2;
				TaskComplete();
			}
			else
			{
...
void CTalkMonster :: RunTask( Task_t_hl1 *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_TLK_CLIENT_STARE:
	case TASK_TLK_LOOK_AT_CLIENT:
			 ...
			 !IsTalking()								)
		{
			...
			pPlayer = g_engfuncsHL1.pfnPEntityOfEntIndex( 1 );
		...
		if ( pTask->iTask == TASK_TLK_CLIENT_STARE )
		{
			...
			if ( UTIL_DotPoints( pPlayer->v.origin, pev->origin, gpGlobalsHL1->v_forward ) < m_flFieldOfView )
			{
				// player looked away
				TaskFail();
			}
		...
		if ( gpGlobalsHL1->time > m_flWaitFinished )
		{
			TaskComplete();
		}
		break;
	...
	case TASK_FACE_PLAYER:
		{
			// Get edict for one player
			edict_t_hl1 *pPlayer = g_engfuncsHL1.pfnPEntityOfEntIndex( 1 );
			...
			if ( pPlayer )
			{
				...
				IdleHeadTurn( pPlayer->v.origin );
				if ( gpGlobalsHL1->time > m_flWaitFinished && FlYawDiff() < 10 )
				{
					TaskComplete();
				}
			}
	...
	case TASK_TLK_EYECONTACT:
		...
		{
			// ALERT( at_console, "waiting %f\n", m_flStopTalkTime - gpGlobalsHL1->time );
...
void CTalkMonster :: Killed( entvars_t *pevAttacker, int iGib )
{
	...
	{
		AlertFriends();
		LimitFollowers( CBaseEntityHL1::Instance(pevAttacker), 0 );
	}
...
CBaseEntityHL1	*CTalkMonster::EnumFriends( CBaseEntityHL1 *pPrevious, int listNumber, BOOL bTrace )
{
	CBaseEntityHL1 *pFriend = pPrevious;
	char *pszFriend;
	TraceResult tr;
	VectorHL1 vecCheck;
...
void CTalkMonster::LimitFollowers( CBaseEntityHL1 *pPlayer, int maxFollowers )
{
	CBaseEntityHL1 *pFriend = NULL;
	int i, count;
...
// nearest player
//=========================================================
CBaseEntityHL1 *CTalkMonster :: FindNearestFriend(BOOL fPlayer)
{
	CBaseEntityHL1 *pFriend = NULL;
	CBaseEntityHL1 *pNearest = NULL;
	float range = 10000000.0;
	TraceResult tr;
	VectorHL1 vecStart = pev->origin;
	VectorHL1 vecCheck;
	int i;
	char *pszFriend;
	int cfriends;
...
void CTalkMonster :: Touch( CBaseEntityHL1 *pOther )
{
...
int CTalkMonster :: FOkToSpeak( void )
{
	...
	if (gpGlobalsHL1->time <= CTalkMonster::g_talkWaitTime)
		return FALSE;
...
int CTalkMonster :: FIdleHello( void )
{
	...
	{
		// get a player
		CBaseEntityHL1 *pPlayer = FindNearestFriend(TRUE);
...
void CTalkMonster :: IdleHeadTurn( VectorHL1 &vecFriend )
{
...
{ 
	...
	CBaseEntityHL1 *pTarget = m_hTargetEnt;
	...
	CBaseEntityHL1 *pFriend = FindNearestFriend(FALSE);
	...
	if ( RANDOM_LONG(0,1) )
	{
		...
		CBaseEntityHL1 *pFriend = FindNearestFriend(TRUE);
...
void CTalkMonster::PlayScriptedSentence( const char *pszSentence, float duration, float volume, float attenuation, BOOL bConcurrent, CBaseEntityHL1 *pListener )
{
	if ( !bConcurrent )
		ShutUpFriends();

	ClearConditions( bits_COND_CLIENT_PUSH );	// Forget about moving!  I've got something to say!
	m_useTime = gpGlobalsHL1->time + duration;
...
void CTalkMonster::PlaySentence( const char *pszSentence, float duration, float volume, float attenuation )
{
	...
	CTalkMonster::g_talkWaitTime = gpGlobalsHL1->time + duration + 2.0;
	if ( pszSentence[0] == '!' )
...
void CTalkMonster :: Talk( float flDuration )
{
	...
	{
		// no duration :( 
		m_flStopTalkTime = gpGlobalsHL1->time + 3;
	}
	else
	{
		m_flStopTalkTime = gpGlobalsHL1->time + flDuration;
	}
}
	...
	case SCHED_IDLE_STAND:
		{	
			...
			if (!FBitSet(m_bitsSaid, bit_saidWoundLight) && (pev->health <= (pev->max_health * 0.75)))
			{
				...
				//CTalkMonster::g_talkWaitTime = gpGlobalsHL1->time + RANDOM_FLOAT(2.8, 3.2);
			...
			if ( !IsTalking() && HasConditions ( bits_COND_SEE_CLIENT ) && RANDOM_LONG( 0, 6 ) == 0 )
			{
				...
				if ( pPlayer )
				{
					...
					UTIL_MakeVectors ( pPlayer->v.angles );
					if ( ( pPlayer->v.origin - pev->origin ).Length2D() < TLK_STARE_DIST	&& 
						 UTIL_DotPoints( pPlayer->v.origin, pev->origin, gpGlobalsHL1->v_forward ) >= m_flFieldOfView )
					{
...
{
	if ( m_flStopTalkTime > gpGlobalsHL1->time )
	{
		return TRUE;
	}
...
void CTalkMonster :: TrySmellTalk( void )
{
	...
	if ( gpGlobalsHL1->time > m_flLastSaidSmelled  )
	{
//		ALERT ( at_aiconsole, "Clear smell bits\n" );
	...
	if (!FBitSet(m_bitsSaid, bit_saidSmelled) && HasConditions ( bits_COND_SMELL ))
	{
		...
		m_flLastSaidSmelled = gpGlobalsHL1->time + 60;// don't talk about the stinky for a while.
...
int CTalkMonster::IRelationship( CBaseEntityHL1 *pTarget )
{
	if ( pTarget->IsPlayer() )
		if ( m_afMemory & bits_MEMORY_PROVOKED )
			return R_HT;
...
void CTalkMonster::StartFollowing( CBaseEntityHL1 *pLeader )
{
	if ( m_pCine )
...
void CTalkMonster :: FollowerUse( CBaseEntityHL1 *pActivator, CBaseEntityHL1 *pCaller, USE_TYPE useType, float value )
{
	// Don't allow use during a scripted_sentence
	if ( m_useTime > gpGlobalsHL1->time )
		return;
...
{
	if ( m_iszUse )
		m_szGrp[TLK_USE] = STRING_HL1( m_iszUse );
	if ( m_iszUnUse )
		m_szGrp[TLK_UNUSE] = STRING_HL1( m_iszUnUse );
}
