...
public:
	void			TalkInit( void );				
	CBaseEntityHL1	*FindNearestFriend(BOOL fPlayer);
	...
	int				IRelationship ( CBaseEntityHL1 *pTarget );
	virtual int		CanPlaySentence( BOOL fDisregardState );
	virtual void	PlaySentence( const char *pszSentence, float duration, float volume, float attenuation );
	void			PlayScriptedSentence( const char *pszSentence, float duration, float volume, float attenuation, BOOL bConcurrent, CBaseEntityHL1 *pListener );
	void			KeyValue( KeyValueData *pkvd );

	// AI functions
	void			SetActivity ( ActivityHL1 newActivity );
	...
	int				FIdleStare( void );
	int				FIdleHello( void );
	void			IdleHeadTurn( VectorHL1 &vecFriend );
	int				FOkToSpeak( void );
	void			TrySmellTalk( void );
	CBaseEntityHL1		*EnumFriends( CBaseEntityHL1 *pentPrevious, int listNumber, BOOL bTrace );
	...
	void			StartFollowing( CBaseEntityHL1 *pLeader );
	virtual void	DeclineFollowing( void ) {}
	void			LimitFollowers( CBaseEntityHL1 *pPlayer, int maxFollowers );

	void EXPORT		FollowerUse( CBaseEntityHL1 *pActivator, CBaseEntityHL1 *pCaller, USE_TYPE useType, float value );
	...
	EHANDLEHL1	m_hTalkTarget;	// who to look at while talking
	CUSTOM_SCHEDULES;
};
