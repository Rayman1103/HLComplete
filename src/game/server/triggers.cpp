...
class CMultiManager : public CBaseToggleHL1
{
public:
	void KeyValue( KeyValueData *pkvd );
	void Spawn ( void );
	void EXPORT ManagerThink ( void );
	void EXPORT ManagerUse   ( CBaseEntityHL1 *pActivator, CBaseEntityHL1 *pCaller, USE_TYPE useType, float value );
...
IMPLEMENT_SAVERESTORE(CMultiManager,CBaseToggleHL1);
...
{
	pev->solid = SOLID_NOT;
	SetUseHL1 ( ManagerUse );
	SetThinkHL1 ( ManagerThink);
...
{ 
	for ( int i = 0; i < m_cTargets; i++ )
		if ( FStrEq(STRING_HL1(targetname), STRING_HL1(m_iTargetName[i])) )
			return TRUE;
...
void CMultiManager :: ManagerThink ( void )
{
	...
	time = gpGlobalsHL1->time - m_startTime;
	while ( m_index < m_cTargets && m_flTargetDelay[ m_index ] <= time )
	{
		FireTargets( STRING_HL1( m_iTargetName[ m_index ] ), m_hActivator, this, USE_TOGGLE, 0 );
		m_index++;
	}
	...
	if ( m_index >= m_cTargets )// have we fired all targets?
	{
		...
		SetUseHL1 ( ManagerUse );// allow manager re-use 
	}
	else
...
void CMultiManager :: ManagerUse ( CBaseEntityHL1 *pActivator, CBaseEntityHL1 *pCaller, USE_TYPE useType, float value )
{
	...
	m_hActivator = pActivator;
	m_index = 0;
	m_startTime = gpGlobalsHL1->time;

	SetUseHL1( NULL );// disable use until all targets have fired
...
void CMultiManager :: ManagerReport ( void )
{
	...
	{
		ALERT ( at_console, "%s %f\n", STRING_HL1(m_iTargetName[cIndex]), m_flTargetDelay[cIndex] );
	}
}
#endif
