...
public:
	...
	virtual void Think( void ) = 0;// GR_Think - runs every server frame, should handle any timer tasks, periodic events, etc.
	virtual BOOL IsAllowedToSpawn( CBaseEntityHL1 *pEntity ) = 0;  // Can this item spawn (eg monsters don't spawn in deathmatch).

	virtual BOOL FAllowFlashlight( void ) = 0;// Are players allowed to switch on their flashlight?
	virtual BOOL FShouldSwitchWeapon( CBasePlayerHL1 *pPlayer, CBasePlayerItemHL1 *pWeapon ) = 0;// should the player switch to this weapon?
	virtual BOOL GetNextBestWeapon( CBasePlayerHL1 *pPlayer, CBasePlayerItemHL1 *pCurrentWeapon ) = 0;// I can't use this weapon anymore, get me the next best one.
...
// Client connection/disconnection
	virtual BOOL ClientConnected( edict_t_hl1 *pEntity, const char *pszName, const char *pszAddress, char szRejectReason[ 128 ] ) = 0;// a client just connected to the server (player hasn't spawned yet)
	virtual void InitHUD( CBasePlayerHL1 *pl ) = 0;		// the client dll is ready for updating
	virtual void ClientDisconnected( edict_t_hl1 *pClient ) = 0;// a client just disconnected from the server
	virtual void UpdateGameMode( CBasePlayerHL1 *pPlayer ) {}  // the client needs to be informed of the current game mode

// Client damage rules
	virtual float FlPlayerFallDamage( CBasePlayerHL1 *pPlayer ) = 0;// this client just hit the ground after a fall. How much damage?
	virtual BOOL  FPlayerCanTakeDamage( CBasePlayerHL1 *pPlayer, CBaseEntityHL1 *pAttacker ) {return TRUE;};// can this player take damage from this attacker?
	virtual BOOL ShouldAutoAim( CBasePlayerHL1 *pPlayer, edict_t_hl1 *target ) { return TRUE; }

// Client spawn/respawn control
	virtual void PlayerSpawn( CBasePlayerHL1 *pPlayer ) = 0;// called by CBasePlayer::Spawn just before releasing player into the game
	virtual void PlayerThink( CBasePlayerHL1 *pPlayer ) = 0; // called by CBasePlayer::PreThink every frame, before physics are run and after keys are accepted
	virtual BOOL FPlayerCanRespawn( CBasePlayerHL1 *pPlayer ) = 0;// is this player allowed to respawn now?
	virtual float FlPlayerSpawnTime( CBasePlayerHL1 *pPlayer ) = 0;// When in the future will this player be able to spawn?
	virtual edict_t_hl1 *GetPlayerSpawnSpot( CBasePlayerHL1 *pPlayer );// Place this player on their spawnspot and face them the proper direction.

	virtual BOOL AllowAutoTargetCrosshair( void ) { return TRUE; };
	virtual BOOL ClientCommandHL1( CBasePlayerHL1 *pPlayer, const char *pcmd ) { return FALSE; };  // handles the user commands;  returns TRUE if command handled properly
	virtual void ClientUserInfoChanged( CBasePlayerHL1 *pPlayer, char *infobuffer ) {}		// the player has changed userinfo;  can change it now

// Client kills/scoring
	virtual int IPointsForKill( CBasePlayerHL1 *pAttacker, CBasePlayerHL1 *pKilled ) = 0;// how many points do I award whoever kills this player?
	virtual void PlayerKilled( CBasePlayerHL1 *pVictim, entvars_t *pKiller, entvars_t *pInflictor ) = 0;// Called each time a player dies
	virtual void DeathNotice( CBasePlayerHL1 *pVictim, entvars_t *pKiller, entvars_t *pInflictor )=  0;// Call this from within a GameRules class to report an obituary.
// Weapon retrieval
	virtual BOOL CanHavePlayerItem( CBasePlayerHL1 *pPlayer, CBasePlayerItemHL1 *pWeapon );// The player is touching an CBasePlayerItem, do I give it to him?
	virtual void PlayerGotWeapon( CBasePlayerHL1 *pPlayer, CBasePlayerItemHL1 *pWeapon ) = 0;// Called each time a player picks up a weapon from the ground

// Weapon spawn/respawn control
	virtual int WeaponShouldRespawn( CBasePlayerItemHL1 *pWeapon ) = 0;// should this weapon respawn?
	virtual float FlWeaponRespawnTime( CBasePlayerItemHL1 *pWeapon ) = 0;// when may this weapon respawn?
	virtual float FlWeaponTryRespawn( CBasePlayerItemHL1 *pWeapon ) = 0; // can i respawn now,  and if not, when should i try again?
	virtual VectorHL1 VecWeaponRespawnSpotHL1( CBasePlayerItemHL1 *pWeapon ) = 0;// where in the world should this weapon respawn?

// Item retrieval
	virtual BOOL CanHaveItem( CBasePlayerHL1 *pPlayer, CItemHL1 *pItem ) = 0;// is this player allowed to take this item?
	virtual void PlayerGotItem( CBasePlayerHL1 *pPlayer, CItemHL1 *pItem ) = 0;// call each time a player picks up an item (battery, healthkit, longjump)

// Item spawn/respawn control
	virtual int ItemShouldRespawn( CItemHL1 *pItem ) = 0;// Should this item respawn?
	virtual float FlItemRespawnTime( CItemHL1 *pItem ) = 0;// when may this item respawn?
	virtual VectorHL1 VecItemRespawnSpotHL1( CItemHL1 *pItem ) = 0;// where in the world should this item respawn?

// Ammo retrieval
	virtual BOOL CanHaveAmmo( CBasePlayerHL1 *pPlayer, const char *pszAmmoName, int iMaxCarry );// can this player take more of this ammo?
	virtual void PlayerGotAmmo( CBasePlayerHL1 *pPlayer, char *szName, int iCount ) = 0;// called each time a player picks up some ammo in the world

// Ammo spawn/respawn control
	virtual int AmmoShouldRespawn( CBasePlayerAmmoHL1 *pAmmo ) = 0;// should this ammo item respawn?
	virtual float FlAmmoRespawnTime( CBasePlayerAmmoHL1 *pAmmo ) = 0;// when should this ammo item respawn?
	virtual VectorHL1 VecAmmoRespawnSpotHL1( CBasePlayerAmmoHL1 *pAmmo ) = 0;// where in the world should this ammo item respawn?
...
// What happens to a dead player's weapons
	virtual int DeadPlayerWeapons( CBasePlayerHL1 *pPlayer ) = 0;// what do I do with a player's weapons when he's killed?

// What happens to a dead player's ammo	
	virtual int DeadPlayerAmmo( CBasePlayerHL1 *pPlayer ) = 0;// Do I drop ammo when the player dies? How much?

// Teamplay stuff
	virtual const char *GetTeamID( CBaseEntityHL1 *pEntity ) = 0;// what team is this entity on?
	virtual int PlayerRelationship( CBaseEntityHL1 *pPlayer, CBaseEntityHL1 *pTarget ) = 0;// What is the player's relationship with this entity?
	virtual int GetTeamIndex( const char *pTeamName ) { return -1; }
	virtual const char *GetIndexedTeamName( int teamIndex ) { return ""; }
	virtual BOOL IsValidTeam( const char *pTeamName ) { return TRUE; }
	virtual void ChangePlayerTeam( CBasePlayerHL1 *pPlayer, const char *pTeamName, BOOL bKill, BOOL bGib ) {}
	virtual const char *SetDefaultPlayerTeam( CBasePlayerHL1 *pPlayer ) { return ""; }

// Sounds
	virtual BOOL PlayTextureSounds( void ) { return TRUE; }
	virtual BOOL PlayFootstepSounds( CBasePlayerHL1 *pl, float fvol ) { return TRUE; }
...
extern CGameRulesHL1 *InstallGameRulesHL1( void );
...
// game.
//=========================================================
class CHalfLifeRules : public CGameRulesHL1
{
public:
	CHalfLifeRules ( void );
...
// GR_Think
	...
	virtual BOOL IsAllowedToSpawn( CBaseEntityHL1 *pEntity );
	virtual BOOL FAllowFlashlight( void ) { return TRUE; };

	virtual BOOL FShouldSwitchWeapon( CBasePlayerHL1 *pPlayer, CBasePlayerItemHL1 *pWeapon );
	virtual BOOL GetNextBestWeapon( CBasePlayerHL1 *pPlayer, CBasePlayerItemHL1 *pCurrentWeapon );
...
// Client connection/disconnection
	virtual BOOL ClientConnected( edict_t_hl1 *pEntity, const char *pszName, const char *pszAddress, char szRejectReason[ 128 ] );
	virtual void InitHUD( CBasePlayerHL1 *pl );		// the client dll is ready for updating
	virtual void ClientDisconnected( edict_t_hl1 *pClient );

// Client damage rules
	virtual float FlPlayerFallDamage( CBasePlayerHL1 *pPlayer );
	
// Client spawn/respawn control
	virtual void PlayerSpawn( CBasePlayerHL1 *pPlayer );
	virtual void PlayerThink( CBasePlayerHL1 *pPlayer );
	virtual BOOL FPlayerCanRespawn( CBasePlayerHL1 *pPlayer );
	virtual float FlPlayerSpawnTime( CBasePlayerHL1 *pPlayer );
...
// Client kills/scoring
	...
	virtual void DeathNotice( CBasePlayerHL1 *pVictim, entvars_t *pKiller, entvars_t *pInflictor );

// Weapon retrieval
	virtual void PlayerGotWeapon( CBasePlayerHL1 *pPlayer, CBasePlayerItemHL1 *pWeapon );

// Weapon spawn/respawn control
	virtual int WeaponShouldRespawn( CBasePlayerItemHL1 *pWeapon );
	virtual float FlWeaponRespawnTime( CBasePlayerItemHL1 *pWeapon );
	virtual float FlWeaponTryRespawn( CBasePlayerItemHL1 *pWeapon );
	virtual VectorHL1 VecWeaponRespawnSpotHL1( CBasePlayerItemHL1 *pWeapon );

// Item retrieval
	virtual BOOL CanHaveItem( CBasePlayerHL1 *pPlayer, CItemHL1 *pItem );
	virtual void PlayerGotItem( CBasePlayerHL1 *pPlayer, CItemHL1 *pItem );

// Item spawn/respawn control
	virtual int ItemShouldRespawn( CItemHL1 *pItem );
	virtual float FlItemRespawnTime( CItemHL1 *pItem );
	virtual VectorHL1 VecItemRespawnSpotHL1( CItemHL1 *pItem );

// Ammo retrieval
	virtual void PlayerGotAmmo( CBasePlayerHL1 *pPlayer, char *szName, int iCount );

// Ammo spawn/respawn control
	virtual int AmmoShouldRespawn( CBasePlayerAmmoHL1 *pAmmo );
	virtual float FlAmmoRespawnTime( CBasePlayerAmmoHL1 *pAmmo );
	virtual VectorHL1 VecAmmoRespawnSpotHL1( CBasePlayerAmmoHL1 *pAmmo );

// Healthcharger respawn control
	virtual float FlHealthChargerRechargeTime( void );

// What happens to a dead player's weapons
	virtual int DeadPlayerWeapons( CBasePlayerHL1 *pPlayer );

// What happens to a dead player's ammo	
	virtual int DeadPlayerAmmo( CBasePlayerHL1 *pPlayer );

// Monsters
	virtual BOOL FAllowMonsters( void );

// Teamplay stuff	
	virtual const char *GetTeamID( CBaseEntityHL1 *pEntity ) {return "";};
...
// competition
//=========================================================
class CHalfLifeMultiplay : public CGameRulesHL1
{
public:
	CHalfLifeMultiplay();
...
// Client spawn/respawn control
	...
	virtual BOOL FPlayerCanRespawn( CBasePlayerHL1 *pPlayer );
	virtual float FlPlayerSpawnTime( CBasePlayerHL1 *pPlayer );
	virtual edict_t_hl1 *GetPlayerSpawnSpot( CBasePlayerHL1 *pPlayer );
...
// Teamplay stuff	
	...
	virtual BOOL PlayFootstepSounds( CBasePlayerHL1 *pl, float fvol );
...
protected:
	...
	void SendMOTDToClient( edict_t_hl1 *client );
};
