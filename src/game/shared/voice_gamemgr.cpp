...
cvar_t voice_serverdebug_hl1 = {"voice_serverdebug_hl1", "0"};

// Set game rules to allow all clients to talk to each other.
// Muted players still can't talk to each other.
cvar_t sv_alltalk_hl1 = {"sv_alltalk_hl1", "0"};
...
// Find a player with a case-insensitive name search.
static CBasePlayerHL1* FindPlayerByNameHL1(const char *pTestName)
{
	for(int i=1; i <= gpGlobalsHL1->maxClients; i++)
	{
		edict_t_hl1 *pEdict = g_engfuncsHL1.pfnPEntityOfEntIndex(i);
		if(pEdict)
		{
			CBaseEntityHL1 *pEnt = CBaseEntityHL1::Instance(pEdict);
			if(pEnt && pEnt->IsPlayer())
			{			
				const char *pNetName = STRING_HL1(pEnt->pev->netname);
				if(stricmp(pNetName, pTestName) == 0)
				{
					return (CBasePlayerHL1*)pEnt;
				}
			}
		}
	}
...
static void VoiceServerDebug( char const *pFmt, ... )
{
	...
	if( !voice_serverdebug_hl1.value )
		return;
...
CVoiceGameMgrHL1::CVoiceGameMgrHL1()
{
	m_UpdateInterval = 0;
	m_nMaxPlayers = 0;
}
...
bool CVoiceGameMgrHL1::Init(
	IVoiceGameMgrHelperHL1 *pHelper,
	int maxClients)
{		  
	m_pHelper = pHelper;
	m_nMaxPlayers = VOICE_MAX_PLAYERS < maxClients ? VOICE_MAX_PLAYERS : maxClients;
	g_engfuncsHL1.pfnPrecacheModel("sprites/voiceicon.spr");
	...
	if ( !CVAR_GET_POINTER( "voice_serverdebug_hl1" ) )
		CVAR_REGISTER( &voice_serverdebug_hl1 );

	if( !CVAR_GET_POINTER( "sv_alltalk_hl1" ) )
		CVAR_REGISTER( &sv_alltalk_hl1 );

	return true;
}


void CVoiceGameMgrHL1::SetHelper(IVoiceGameMgrHelperHL1 *pHelper)
{
	m_pHelper = pHelper;
}


void CVoiceGameMgrHL1::Update(double frametime)
{
	// Only update periodically.
	m_UpdateInterval += frametime;
	if(m_UpdateInterval < UPDATE_INTERVAL)
...
void CVoiceGameMgrHL1::ClientConnected(edict_t_hl1 *pEdict)
{
	int index = ENTINDEXHL1(pEdict) - 1;
...
bool CVoiceGameMgrHL1::PlayerHasBlockedPlayer(CBasePlayerHL1 *pReceiver, CBasePlayerHL1 *pSender)
{
...
bool CVoiceGameMgrHL1::ClientCommand(CBasePlayerHL1 *pPlayer, const char *cmd)
{
	int playerClientIndex = pPlayer->entindex() - 1;
	if(playerClientIndex < 0 || playerClientIndex >= m_nMaxPlayers)
	{
...
void CVoiceGameMgrHL1::UpdateMasks()
{
	m_UpdateInterval = 0;

	bool bAllTalk = !!g_engfuncsHL1.pfnCVarGetFloat( "sv_alltalk_hl1" );

	for(int iClient=0; iClient < m_nMaxPlayers; iClient++)
	{
		CBaseEntityHL1 *pEnt = UTIL_PlayerByIndexHL1(iClient+1);
		if(!pEnt || !pEnt->IsPlayer())
			continue;
		...
		CBasePlayerHL1 *pPlayer = (CBasePlayerHL1*)pEnt;
		...
		if( g_PlayerModEnable[iClient] )
		{
			...
			for(int iOtherClient=0; iOtherClient < m_nMaxPlayers; iOtherClient++)
			{
				CBaseEntity *pEnt = UTIL_PlayerByIndex(iOtherClient+1);
				if(pEnt && pEnt->IsPlayer() && 
					(bAllTalk || m_pHelper->CanPlayerHearPlayer(pPlayer, (CBasePlayerHL1*)pEnt)) )
				{
					gameRulesMask[iOtherClient] = true;
				}
			}
		...
		{
			bool bCanHear = gameRulesMask[iOtherClient] && !g_BanMasks[iClient][iOtherClient];
			g_engfuncsHL1.pfnVoice_SetClientListening(iClient+1, iOtherClient+1, bCanHear);
		}
	}
}