...
#ifdef DOUBLEVEC_T
typedef double vec_t_hl1;
#else
typedef float vec_t_hl1;
#endif
typedef vec_t_hl1 vec3_t[3];	// x,y,z
typedef vec_t_hl1 vec4_t[4];	// x,y,z,w
...
#define DotProductHL1(x,y) ((x)[0]*(y)[0]+(x)[1]*(y)[1]+(x)[2]*(y)[2])
#define VectorFillHL1(a,b) { (a)[0]=(b); (a)[1]=(b); (a)[2]=(b);}
#define VectorAvgHL1(a) ( ( (a)[0] + (a)[1] + (a)[2] ) / 3 )
#define VectorSubtractHL1(a,b,c) {(c)[0]=(a)[0]-(b)[0];(c)[1]=(a)[1]-(b)[1];(c)[2]=(a)[2]-(b)[2];}
#define VectorAddHL1(a,b,c) {(c)[0]=(a)[0]+(b)[0];(c)[1]=(a)[1]+(b)[1];(c)[2]=(a)[2]+(b)[2];}
#define VectorCopyHL1(a,b) {(b)[0]=(a)[0];(b)[1]=(a)[1];(b)[2]=(a)[2];}
#define VectorScaleHL1(a,b,c) {(c)[0]=(b)*(a)[0];(c)[1]=(b)*(a)[1];(c)[2]=(b)*(a)[2];}

vec_t_hl1 Q_rint (vec_t_hl1 in);
vec_t_hl1 _DotProductHL1 (vec3_t v1, vec3_t v2);
void _VectorSubtractHL1 (vec3_t va, vec3_t vb, vec3_t out);
void _VectorAddHL1 (vec3_t va, vec3_t vb, vec3_t out);
void _VectorCopyHL1 (vec3_t in, vec3_t out);
void _VectorScaleHL1 (vec3_t v, vec_t_hl1 scale, vec3_t out);

double VectorLengthHL1(vec3_t v);

void VectorMAHL1 (vec3_t va, double scale, vec3_t vb, vec3_t vc);

void CrossProductHL1 (vec3_t v1, vec3_t v2, vec3_t cross);
vec_t_hl1 VectorNormalizeHL1 (vec3_t v);
void VectorInverseHL1 (vec3_t v);

void ClearBoundsHL1 (vec3_t mins, vec3_t maxs);
void AddPointToBoundsHL1 (vec3_t v, vec3_t mins, vec3_t maxs);

void AngleMatrixHL1 (const vec3_t angles, float matrix[3][4] );
void AngleIMatrixHL1 (const vec3_t angles, float matrix[3][4] );
void R_ConcatTransformsHL1 (const float in1[3][4], const float in2[3][4], float out[3][4]);

void VectorIRotateHL1 (const vec3_t in1, const float in2[3][4], vec3_t out);
void VectorRotateHL1 (const vec3_t in1, const float in2[3][4], vec3_t out);

void VectorTransformHL1 (const vec3_t in1, const float in2[3][4], vec3_t out);

void AngleQuaternionHL1( const vec3_t angles, vec4_t quaternion );
void QuaternionMatrixHL1( const vec4_t quaternion, float (*matrix)[4] );
void QuaternionSlerpHL1( const vec4_t p, vec4_t q, float t, vec4_t qt );
